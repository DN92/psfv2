...
export default function ProgressBar( { currentStep, updateStepState, userStartedForm, slots = 3 }: ComponentProps ): JSX.Element {
  ...
  progressBarDividers.push( <div key="gutter1" className={`${styles.progress_step_gutter} ${styles.progress_step_gutter_start}`} /> );
  progressBarDividers.push( <div key="break-initial" className={styles.progress_break} /> );
  ...
  for ( let i = 0; i < slots + 1; i++ ) {
    ...
    progressBarDividers.push(
      <div key={`break${i}`} className={styles.progress_break} />,
    );
  }

  progressBarDividers.push( <div key="gutter2" className={`${styles.progress_step_gutter} ${styles.progress_step_gutter_end}`} /> );
