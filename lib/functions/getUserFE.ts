...
export async function getUserFE(setterFunction?:Dispatch<ExtendedUser>): Promise<null | ExtendedUser> {
  ...
  const { data: permissions, error: permissionsError } = await supabase.from('user_permissions').select('id, level').match({ id: session.user.id }).single();

  const extendedUser: ExtendedUser = { ...session.user, permissions: permissions.level ?? '' };

  console.log('getUserFE user:: ', extendedUser);

  setterFunction(extendedUser);
